...
void Camera::applyGammaCorrection(cv::cuda::GpuMat& gpu_image, float gamma) {
...
void Camera::saveHistogramImage(void* data, int width, int height, const std::string& histogramImageFilename, const std::string& histogramCsvFilename) {

    auto* rawData = reinterpret_cast<uint8_t*>(data);

        // 히스토그램 초기화
    std::vector<int> histogramR(1024, 0);
    std::vector<int> histogramG(1024, 0);
    std::vector<int> histogramB(1024, 0);

    printf("File: %s | Line: %d | Function: %s | Message: %s\033[0m\n", __FILE__, __LINE__, __FUNCTION__,"ohh" );
    // 히스토그램 계산
    for (int row = 0; row < height; ++row) {
        size_t offset = row * width * 2; // 한 행당 2바이트(16비트) × 픽셀

        if (row % 2 == 0) { // 짝수 행 (G, B)
            auto* pixels = reinterpret_cast<rg10_pixel_even*>(rawData + offset);
            for (int col = 0; col < width / 2; ++col) { // 2픽셀씩 처리
                histogramB[pixels[col].b]++;
                histogramG[pixels[col].g]++;
            }
        } else { // 홀수 행 (R, G)
            auto* pixels = reinterpret_cast<rg10_pixel_odd*>(rawData + offset);
            for (int col = 0; col < width / 2; ++col) { // 2픽셀씩 처리
                histogramR[pixels[col].r]++;
                histogramG[pixels[col].g]++;
            }
        }
    }
printf("File: %s | Line: %d | Function: %s | Message: %s\033[0m\n", __FILE__, __LINE__, __FUNCTION__,"ohh" );
    ...
    // 히스토그램 이미지 생성 (생략, 위 코드와 동일)
    int histWidth = 1024; // 히스토그램 이미지 너비
    int histHeight = 400; // 히스토그램 이미지 높이
    cv::Mat histImage(histHeight, histWidth, CV_8UC3, cv::Scalar(0, 0, 0));

    // 각 채널의 최대값 계산
    int maxCountR = *std::max_element(histogramR.begin(), histogramR.end());
    int maxCountG = *std::max_element(histogramG.begin(), histogramG.end());
    int maxCountB = *std::max_element(histogramB.begin(), histogramB.end());

    // 히스토그램 데이터 정규화 및 시각화
    for (int i = 0; i < histWidth; ++i) {
        int binStart = i * (1024 / histWidth);
        int binEnd = (i + 1) * (1024 / histWidth);
        ...
        int rHeight = static_cast<int>(static_cast<double>(rSum) / maxCountR * histHeight);
        int gHeight = static_cast<int>(static_cast<double>(gSum) / maxCountG * histHeight);
        int bHeight = static_cast<int>(static_cast<double>(bSum) / maxCountB * histHeight);

        // R 채널 (빨강)
        cv::line(histImage, cv::Point(i, histHeight), cv::Point(i, histHeight - rHeight), cv::Scalar(0, 0, 255));
        // G 채널 (초록)
        cv::line(histImage, cv::Point(i, histHeight), cv::Point(i, histHeight - gHeight), cv::Scalar(0, 255, 0));
        // B 채널 (파랑)
        cv::line(histImage, cv::Point(i, histHeight), cv::Point(i, histHeight - bHeight), cv::Scalar(255, 0, 0));
    }
...
bool Camera::captureOpencv(void) {
    struct v4l2_buffer buf{};
    buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;
...
cv::Mat Camera::convertRGGBToMat(void* data, int width, int height) {
  // RGGB 데이터 크기 계산
    size_t dataSize = width * height * 2; // 2 bytes per pixel
    auto* rawData = reinterpret_cast<uint8_t*>(data);
    ...
    // RGGB 패턴 데이터를 저장할 Mat 객체 생성 (CV_8UC1, RGGB 패턴 그대로 유지)
    cv::Mat resultMat(height, width, CV_8UC1);

    // RGGB 데이터를 8비트로 변환하여 Mat 객체에 저장
    for (int row = 0; row < height; ++row) {
        for (int col = 0; col < width; ++col) {
            ...
            uint16_t low = rawData[offset];
            uint16_t high = rawData[offset + 1];

            // 유효한 10비트를 추출하고 8비트로 변환 (>> 2)
            uint8_t pixelValue = ((high << 8 | low) & 0x03FF) >> 2;

            // Mat 객체에 저장
            resultMat.at<uint8_t>(row, col) = pixelValue;
        }
    }
...
void Camera::processRawImageCUDA(void* data, int width, int height) {
    ...
    //cv::Mat rggbMat = convertRGGBToMat(data, width, height);

     // 디베이어링 추가
    //cv::Mat colorMat3;
    //cv::cvtColor(rggbMat, colorMat3, cv::COLOR_BayerRG2BGR);
    ...
    cv::imshow("gama", colorMat1);

    // 상위 비트를 제거한 후 디베이어링
    //cv::Mat scaledMat2(rawMat.size(), rawMat.type());
    //for (int i = 0; i < rawMat.rows; ++i) {
    //    for (int j = 0; j < rawMat.cols; ++j) {
    //        scaledMat2.at<uint16_t>(i, j) = rawMat.at<uint16_t>(i, j) & 0x03FF;
    //    }
   // }
//     for (int i = 0; i < rawMat.rows; ++i) {
//
//         for (int j = 1; j < rawMat.cols; ++j) {
//             scaledMat2.at<uint16_t>(i, j) = rawMat.at<uint16_t>(i, j) & 0x03FF;
//         }
//     }
//
//     scaledMat2.convertTo(scaledMat2, CV_8U, 255.0 / 1023.0);
//
//     cv::Mat colorMat2;
//     cv::cvtColor(scaledMat2, colorMat2, cv::COLOR_BayerRG2BGR);
//
...
//     }
    ...
    //if(image_count >= 199) saveFrameToFile(raw, width*height*sizeof(uint16_t));
    // 16bit -> 8bit falied....
    //std::vector<uint8_t> output_image(width*height);
    //convert_10bit_to_8bit(raw,output_image.data(),width,height);
...
//
    ...
    // 감마 보정 적용
    //applyGammaCorrection(rgb_image_gpu, 2.2);
